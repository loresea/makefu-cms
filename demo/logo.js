...
function makeIndustryLogoSvg(name,accent='#f97316',family=''){
  ...
  return '<svg xmlns="http://www.w3.org/2000/svg" width="310" height="48" viewBox="0 0 310 48" role="img" aria-label="'+escapeLogoText(text)+'">'+
    ...
    '<text x="54" y="22" font-family="Arial, PingFang SC, Microsoft YaHei, sans-serif" font-size="16.5" font-weight="800" fill="#0f172a">'+escapeLogoText(text)+'</text>'+
    '<text x="54" y="37" font-family="Arial, sans-serif" font-size="8.2" font-weight="700" letter-spacing="1.25" fill="#94a3b8">MAKEFU · '+escapeLogoText(familyText.slice(0,24))+'</text>'+
  '</svg>';
}
function makeIndustryLogo(name,accent='#f97316',family=''){
  const key=[name,accent,family].join('|');if(logoCache.has(key))return logoCache.get(key);
  try{const svg=makeIndustryLogoSvg(name,accent,family);const url=URL.createObjectURL(new Blob([svg],{type:'image/svg+xml'}));logoCache.set(key,url);return url}catch(e){
    const fallback='<svg xmlns="http://www.w3.org/2000/svg" width="250" height="48"><rect x="4" y="4" width="40" height="40" rx="10" fill="'+accent+'"/><text x="24" y="31" text-anchor="middle" font-family="Arial" font-size="17" font-weight="800" fill="white">M</text><text x="55" y="29" font-family="Arial,PingFang SC,Microsoft YaHei" font-size="16" font-weight="800" fill="#0f172a">'+escapeLogoText(name||'码科服')+'</text></svg>';
    return 'data:image/svg+xml;charset=UTF-8,'+encodeURIComponent(fallback);
  }
}
function industryLogoMarkup(name,accent,family,cls=''){
  return '<img class="'+cls+'" src="'+makeIndustryLogo(name,accent,family)+'" alt="'+String(name).replace(/"/g,'')+' Logo" onerror="this.onerror=null;this.replaceWith(Object.assign(document.createElement(\'span\'),{className:\'logoFallback\',textContent:\''+String(name).replace(/'/g,'')+'\'}))">';
}
